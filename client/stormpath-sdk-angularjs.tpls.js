...
angular.module("spRegistrationForm.tpl.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("spRegistrationForm.tpl.html",
    ...
    "    background-color: #C74539;\n" +
    "    border-color: #AF4138;\n" +
    "  }</style><div class=row><div class=\"col-sm-12\"><p class=\"alert alert-success\" ng-show=\"created && !enabled\">Your account has been created. Please check your email for a verification link.</p><p ng-show=\"created && !enabled\" class=pull-right><a href=/login ng-click=hideSignup()>Back to Login</a></p><p class=\"alert alert-success\" ng-show=\"created && enabled && !authenticating\">Your account has been created. <a href=/login ng-click=hideSignup()>Login Now</a>.</p></div></div><div class=row><div class=col-xs-12><form ng-hide=\"created && !authenticating\" ng-submit=submit()><div class=form-group><p class=\"alert alert-danger\" ng-show=error ng-bind=error></p></div><div class=form-group><input class=form-control id=spFirstName ng-model=formModel.givenName placeholder=\"First Name\" ng-disabled=creating></div><div class=form-group><input class=form-control id=spLastName ng-model=formModel.surname placeholder=\"Last Name\" ng-disabled=creating></div><div class=form-group><input class=form-control id=spEmail ng-model=formModel.email placeholder=Email ng-disabled=creating></div><div class=form-group><input type=password class=form-control id=spPassword ng-model=formModel.password placeholder=Password ng-disabled=creating></div><div class=form-group><button type=submit class=\"btn btn-primary\" ng-disabled=creating>Sign up</button></div><div class=form-group ng-show=socialLoginProviders.length><p>Or register with:</p><button ng-repeat=\"provider in socialLoginProviders\" type=button class=\"btn btn-social btn-{{provider.name}}\" sp-social-login={{provider.name}}>{{provider.service.name}}</button></div></form></div></div>");
}]);
